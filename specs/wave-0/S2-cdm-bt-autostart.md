...
- **S2-AC1** — WHEN сопряжённые наушники подключаются по BT, THE SYSTEM SHALL легально стартовать сервис захвата из фона без открытия UI, с готовностью к записи **≤ 3 с** от события BT-подключения (порог — документированное допущение, на ратификацию в PR фазы).
...
## Device-матрица
> OEM-набор ратифицирован фаундером (Q3 закрыт, grill 2026-07-07 §4.2): FTL Pixel/Samsung + физ-Xiaomi обязателен + Docker-эмулятор как smoke-ярус.

| Устройство | OEM / ярус | Фокус | Результат |
|---|---|---|---|
| Docker-эмулятор (у фаундера) | smoke | быстрая петля: CDM-ассоциация + старт/стоп | — |
| Pixel 8 | Google (FTL) | базовый CDM-фон-старт | — |
| Galaxy | Samsung (FTL) | BT-стек + фон | — |
| Redmi/POCO | Xiaomi (физ, батчем) | автозапуск под MIUI-ограничениями | — |
...
## Evidence (ADR-010)
Спайк по определению — live. Прикладывать в `specs/wave-0/evidence/WAVE0-S2/`:
- device-логи старта/стопа сервиса по `ACTION_ACL_CONNECTED` (logcat, вкл. замер BT-connect → готовность к записи ≤3 с);
- прогоны по device-матрице (Docker-эмулятор smoke → FTL Pixel/Samsung → физ-Xiaomi), ссылки на FTL-раны;
- негативный кейс AC3 (отключение наушников → стоп).
Go/No-Go подтверждается **evidence**, не утверждением. Live невозможен (нет FTL-кредов/девайса) → явный `evidence_gap: needs-device|needs-FTL-creds`, не тихий скип.
