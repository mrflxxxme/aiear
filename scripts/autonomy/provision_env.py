#!/usr/bin/env python3
"""Provision the funded ``.env`` into the active worktree (ADR-011 preflight).

The autonomous runner may work in a fresh git worktree per run. Live-gold gates
need a funded ``.env`` (Yandex SpeechKit / GigaChat / SaluteSpeech / RuStore
Pay sandbox keys), but secrets must NEVER be committed. This helper copies the
single **canonical** funded env — kept at the MAIN worktree's ``.env``
(git-ignored, the one checkout that persists) — into the active worktree's
``.env`` when it is absent.

Security invariants (all enforced, not assumed):
- The destination MUST be git-ignored. If ``git check-ignore`` does not claim
  it, the copy is REFUSED (exit 3) — better a stuck run than a staged secret.
...
Usage (runner preflight, before any live-gold gate):
...
_REL = ".env"
# ФЗ-242 RF-stack providers (see .env.example). Names only — never values.
_PROVIDERS = ("YANDEX", "GIGACHAT", "SALUTESPEECH", "RF_OBJECT_STORAGE", "RUSTORE", "ACQUIRER")
...
def _key_summary(env_path: Path) -> str:
    ...
    for raw in env_path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if key.isupper():
            count += 1
        if key.startswith(_PROVIDERS) and val.strip():
            present.append(key)
