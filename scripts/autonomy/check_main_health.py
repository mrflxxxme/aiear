#!/usr/bin/env python3
"""Main-branch health verdict + offender attribution (ADR-011 D7, Block D).

The autonomous runner (would-)auto-merge; a merged phase can redden ``main``
later (the next phase's CI, a re-run). This script gives the runner a
deterministic verdict: per workflow, take the LATEST completed run on main —
any failure means main is unhealthy, and the failing run's head_sha IS the
offending (squash-)merge commit to revert.
...
# Workflows that gate merges. AIEAR: android + backend + security + evidence.
# (A push-triggered ops/deploy workflow, when one exists, is NOT a merge gate.)
_GATE_WORKFLOWS = ("ci-android", "ci-backend", "ci-security", "ci-evidence")
