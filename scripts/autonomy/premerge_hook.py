#!/usr/bin/env python3
"""PreToolUse hook — tripwire enforcement on `gh pr merge` (ADR-011 D2, Block C).
...
The classifier is stdlib-only, so it runs as bare ``python`` (same interpreter
as this hook) — AIEAR has no guaranteed backend venv.
...
def main() -> int:
    ...
    # --- classify (bare python; the classifier is stdlib-only) ---------------
    classify = _run(
        [sys.executable, "scripts/autonomy/classify_tripwire.py", "--files", *files]
    )
    ...
    return _block(
        ...
        f"(ADR-011 D2). Add a RUN-QUEUE entry (`python scripts/autonomy/run_queue.py add "
