#!/usr/bin/env python3
"""Append a uniform entry to the autonomy decisions-log (ADR-011 D4).
...
Example:
  python scripts/autonomy/log_decision.py --phase WAVE0-S2 --kind impl \\
    --fork "CDM re-bond storage: DataStore vs SharedPreferences" \\
    --decision "DataStore (Proto) — single source for the paired-device id" \\
    --rationale "async-safe, typed; fits Compose; no new dep beyond androidx.datastore" \\
...
_HEADER = (
    ...
    "asking the founder (ADR-011 D4). The founder's post-hoc audit trail. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--adr", default=None, help="ADR ref if architectural, e.g. ADR-012.")
