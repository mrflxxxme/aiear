...
1. **STATUS.md** — обнови статус фазы/волны, гейты, «следующее действие». **+ `.planning/roadmap/phase-queue.yaml`** — обнови `status` закрытой фазы (`done`) и разблокированных ею (`blocked`→`ready`); очередь — источник «что дальше» для runner'а (A6), ты — её единственный писатель.
