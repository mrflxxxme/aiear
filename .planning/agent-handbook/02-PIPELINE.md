...
- Несколько **независимых** фаз могут идти разными автономными сессиями (фаундер авторизовал).
- **Одну фазу можно вести в несколько сессий последовательно** (когда не хватает контекстного окна) — продолжающая сессия **резюмирует** из durable-состояния (STATUS + PLAN-<PHASE> + последний хендоф + собранный evidence), не рестартует и не пропускает стадии (см. [`03-HANDOFF.md`](03-HANDOFF.md) §продолжение). Качество и дисциплина важнее числа сессий.
- **Нельзя** запускать **несколько параллельных** сессий на ОДНОЙ фазе/ветке одновременно — гонка за `phase-state`. Параллелизм — только между независимыми фазами и внутри пайплайна (роли выше).
